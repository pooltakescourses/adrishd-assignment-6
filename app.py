...
def generate_plots(N, mu, sigma2, S):
    ...
    X = np.random.rand(N)  # Generates N values between 0 and 1
    Y = mu + np.sqrt(sigma2) * np.random.randn(N)  # Generates N values with mean mu and variance sigma^2

    # TODO 2: Fit a linear regression model to X and Y
    # Hint: Use Scikit Learn
    model = LinearRegression()
    model.fit(X.reshape(-1, 1), Y)  # Reshape X to be 2D for the model
